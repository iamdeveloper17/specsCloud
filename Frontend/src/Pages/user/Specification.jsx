...
const Specification = () => {
  ...
  useEffect(() => {
    fetchFiles(); // if you already have this
    fetchFolderSuggestions();
  }, []);

  const fetchFolderSuggestions = async () => {
    try {
      const res = await axios.get('https://specscloud-1.onrender.com/api/specification/folder-names');
              ...
              {filteredFiles.length === 0 ? (
                ...
                filteredFiles.map((file) => (
                  <tr key={file._id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 px-2 sm:px-4 max-w-[200px] truncate text-xs sm:text-sm" title={file.fileType}>
                      {file.fileType || 'N/A'}
                    </td>
